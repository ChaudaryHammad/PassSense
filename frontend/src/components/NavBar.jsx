          ...
          {navItems.map(({ to, label }) => {
            ...
            return (
              <li key={to} className="py-3">
                <Link
                  to={to}
                  className={`relative pb-1 ${isActive ? 'text-[#bd7dff] dark:text-[#bd7dff]' : 'text-zinc-400 dark:text-gray-400'}`}
                >
                  {label}
...
<div className="dropdown dropdown-end">
     ...
     {
      user ?   <li><a onClick={logoutHandler}>Logout</a></li>  :  <li>
      ...
      >
        <ul className="flex flex-col items-center py-4 space-y-4 text-gray-800 dark:text-gray-200">
          {navItems.map(({ to, label }) => (
            <li key={to}>
              <Link to={to} className="hover:text-[#bd7dff] dark:hover:text-[#bd7dff]" onClick={toggleMenu}>
                {label}
              </Link>
            </li>
          ))}
          <li>
            <Link to="/login" className="hover:text-gray-500 dark:hover:text-gray-300" onClick={toggleMenu}>
              Login
            </Link>
          </li>
